...
l=InputNumber.b()
e1=ArrayNumLimit.n()
e2=ArrayNumLimit.dates()
e3=ArrayNumLimit.Currenttime()
...
df2 = pd.DataFrame(zip([c,l],e2,e3),index=["String","Number"],columns=["Output","Date","Time"])
df3 = pd.DataFrame(zip(e1,e2,e3),index=[i for i in range(len(ArrayNumLimit.n()))],columns=["Numbers","Date","Time"])
